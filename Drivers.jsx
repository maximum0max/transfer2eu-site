...
const EMPTY_DRIVER = { name: '', phone: '', email: '', car: '', year: '', region: '' };
        ...
        <div style={inner}>
          <form style={formWrap} onSubmit={onSubmit}>
            ...
            {!submitted ? (
              <>
                <div style={fieldRow}>
                  <div style={field}><label style={fieldLabel}>Имя</label>          <input style={fieldInput} type="text" required value={form.name} onChange={e => set('name', e.target.value)} autoComplete="name" /></div>
                  <div style={field}><label style={fieldLabel}>Телефон</label>      <input style={fieldInput} type="tel" required value={form.phone} onChange={e => set('phone', e.target.value)} placeholder="+34 ..." autoComplete="tel" /></div>
                  <div style={field}><label style={fieldLabel}>E-mail</label>       <input style={fieldInput} type="email" required value={form.email} onChange={e => set('email', e.target.value)} autoComplete="email" /></div>
                  <div style={field}><label style={fieldLabel}>Марка авто</label>   <input style={fieldInput} type="text" required value={form.car} onChange={e => set('car', e.target.value)} placeholder="Mercedes V-class" /></div>
                  <div style={field}><label style={fieldLabel}>Год выпуска</label>  <input style={fieldInput} type="number" required min="2010" value={form.year} onChange={e => set('year', e.target.value)} placeholder="2018" /></div>
                  <div style={field}><label style={fieldLabel}>Регион проживания</label><input style={fieldInput} type="text" required value={form.region} onChange={e => set('region', e.target.value)} placeholder="Аликанте, Бенидорм…" /></div>
                </div>
                {/* Honeypot — off-screen; bots fill it, humans don't */}
                <div aria-hidden="true" style={{ position: 'absolute', left: '-9999px', width: 1, height: 1, overflow: 'hidden' }}>
                  <input type="text" tabIndex={-1} autoComplete="off" value={hp} onChange={e => setHp(e.target.value)} />
                </div>
                <button type="submit" style={{ ...submitBtn, opacity: sending ? .7 : 1, cursor: sending ? 'wait' : 'pointer' }} disabled={sending}>
                  {sending ? 'Отправляем…' : 'Отправить анкету'}
                </button>
                {error && <p style={{ ...formSub, color: 'var(--t2-danger)', marginTop: 12, marginBottom: 0 }}>{error}</p>}
              </>
            ) : (

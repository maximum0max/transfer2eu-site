...
import { POPULAR, waLink, wikiThumb, wikiSrcset } from './BrandData.jsx'
...
function PopularRoutes({ onSelectRoute, onNav }) {
  ...
  return (
    <section style={wrap} aria-labelledby="popular-routes-heading">
      ...
      <style>{`
        .t2-route-card { transition: transform 280ms cubic-bezier(.2,.7,.2,1), box-shadow 280ms cubic-bezier(.2,.7,.2,1); }
        .t2-route-img { transition: transform 600ms cubic-bezier(.2,.7,.2,1); }
        @media (hover: hover) {
          .t2-route-card:hover {
            transform: translateY(-4px);
            box-shadow: 0 24px 60px rgba(11,30,51,.18), 0 8px 20px rgba(11,30,51,.10);
          }
          .t2-route-card:hover .t2-route-img { transform: scale(1.06); }
          .t2-route-card:hover .t2-route-btn { background: linear-gradient(135deg, var(--t2-red), #c01928); }
        }
      `}</style>
    </section>
  );
...
function RouteCard({ r, onSelectRoute }) {
  ...
  const card = {
    ...
    boxShadow: '0 4px 14px rgba(11,30,51,.06)',
  ...
  // Top-left emoji badge (no backdrop-filter — expensive on mobile; the white
  // 95% opacity is already plenty for legibility over the photo)
  const emojiBadge = {
  ...
  const incRow = {
    display: 'flex', flexDirection: 'column', gap: 6, fontSize: 11, color: 'var(--t2-ink-3)',
  ...
  return (
    ...
    >
      ...
      <div style={photoBox}>
        {imgOk && r.img && (
          <img
            src={wikiThumb(r.img, 480)}
            srcSet={wikiSrcset(r.img)}
            sizes="(min-width: 1100px) 300px, (min-width: 720px) 45vw, 100vw"
            alt={altText} loading="lazy" decoding="async"
            width="400" height="300"
            className="t2-route-img"
            style={photoImg}
      ...
      <div style={body}>
        <div style={incRow}>
          <span style={{ fontWeight: 700, color: 'var(--t2-ink-2)' }}>Включено:</span>
          <span style={incItem}>🅿 Платные дороги</span>
          <span style={incItem}>👶 Детские кресла</span>
          <span style={incItem}>✈️ Ожидание в аэропорту</span>
        </div>
        <div style={priceRow}>
          <div>
            <div style={priceLabel}>Цена</div>

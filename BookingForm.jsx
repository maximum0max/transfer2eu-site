...
function BookingForm({ initialSlug = '', lockDestination = false }) {
  ...
  const airportDestination = { slug: '__airport__', city: 'Airport', ru: 'Аэропорт Аликанте (ALC)', price: 30, emoji: '✈️', time: 15 };
  ...
  const [form, setForm] = useState({ name: '', phone: '', flight: '', date: '', time: '', passengers: '2', luggage: '2', notes: '' });
  ...
  // 5–8 пассажиров → минивэн (+30€). До 4 — седан (базовая цена).
  const isMinivan = parseInt(form.passengers, 10) > 4;
  const effectivePrice = selected ? (isMinivan ? selected.price + 30 : selected.price) : 0;
  const vehicleLabel = isMinivan ? '🚐 Минивэн' : '🚖 Седан';

  const buildWaUrl = () => {
    const lines = [
      'Здравствуйте! Хочу забронировать трансфер.',
      '',
      `↔️ Маршрут: ${fromLabel} → ${toLabel}`,
      fromMode === 'other' ? '💶 Стоимость: уточним в чате' : `💶 Стоимость: ${effectivePrice}€ (${vehicleLabel})`,
      '',
      `👤 Имя: ${form.name}`,
      `📞 Телефон: ${form.phone}`,
      `📅 Дата: ${form.date}`,
  ...
  /* ---------------- styles ---------------- */
  const card = {
    background: '#fff', borderRadius: 22, overflow: 'visible',
        ...
        <div style={body} className="t2-anim-fade">
          <div className="t2-anim-pop" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16, padding: '11px 14px', borderRadius: 12, background: 'var(--t2-red-soft)', border: '1px solid var(--t2-red)' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0, marginLeft: 8 }}>
              {fromMode === 'other'
                ...
                : <span style={{ display: 'inline-flex', alignItems: 'baseline', gap: 6, fontFamily: "'Onest',sans-serif", fontSize: 16, fontWeight: 800, color: 'var(--t2-red)' }}>
                    {effectivePrice}€
                    {isMinivan && <span style={{ fontSize: 10, fontWeight: 700, color: 'var(--t2-ink-3)', textTransform: 'uppercase', letterSpacing: '.06em' }}>минивэн</span>}
                  </span>}
              {!lockDestination && (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div>
              ...
              <input type="text" value={form.name} onChange={e => setField('name', e.target.value)} placeholder="Александр" autoComplete="name" style={field} />
            </div>
            <div>
            ...
            <div>
              ...
              <p style={hint}>5–8 чел. → минивэн (+30€ к базе) · детские кресла бесплатно</p>
            </div>
            <div>
          ...
          <a
            ...
            <span>{formValid
              ? `Отправить в WhatsApp${fromMode !== 'other' ? ` — ${effectivePrice}€` : ''}`

...
import { BRAND, findRoute, POPULAR, wikiThumb, wikiSrcset, wikiSrcsetWide } from './BrandData.jsx'
...
function RouteHero({ r }) {
  // No overflow:hidden on the wrap — the stat strip below uses negative
  // margin to overlap into the next section; clipping cuts off its labels.
  // We clip the photo inside the dedicated absolute container instead.
  const wrap = {
  ...
  return (
    <section style={wrap}>
      <div style={photo}>
        {r.img && (
          <img
            src={wikiThumb(r.img, 1600)}
            srcSet={wikiSrcsetWide(r.img)}
            sizes="100vw"
            alt={r.alt || ''} fetchpriority="high" decoding="async"
            style={photoImg}
      ...
      <div style={inner}>
        <div style={grid} className="t2-route-grid">
          <div style={{ paddingTop: 24 }}>
            ...
            {r.info?.intro && <p style={{ ...body, fontWeight: 500, color: 'var(--t2-ink)' }}>{r.info.intro}</p>}
            <p style={body}>
              {r.info?.body ||
                'Бронируйте онлайн или в WhatsApp. Цена не меняется от времени суток, пробок или языка водителя. Платные дороги, детское кресло и встреча у выхода — включены.'}
            </p>
...
/* ============ 2b. Google Maps embed with route ============ */
function RouteMap({ r }) {
  const origin = encodeURIComponent('Alicante Airport (ALC), Spain');
  const dest = encodeURIComponent(`${r.city}, Spain`);
  const src = `https://maps.google.com/maps?saddr=${origin}&daddr=${dest}&output=embed`;

  const wrap = { padding: '72px 32px', background: '#fff' };
  const inner = { maxWidth: 1280, margin: '0 auto' };
  const head = { textAlign: 'center', marginBottom: 24 };
  const eyebrow = { fontSize: 12, fontWeight: 700, color: 'var(--t2-red)', letterSpacing: '.12em', textTransform: 'uppercase' };
  const h2 = { fontFamily: "'Onest',sans-serif", fontWeight: 800, fontSize: 'clamp(26px, 3vw, 36px)', letterSpacing: '-.02em', color: 'var(--t2-ink)', margin: '8px 0 6px', lineHeight: 1.1 };
  const lede = { fontSize: 14, color: 'var(--t2-ink-3)', margin: 0 };
  const frameWrap = {
    borderRadius: 18, overflow: 'hidden',
    border: '1px solid var(--t2-line)', boxShadow: 'var(--t2-sh-1)',
    aspectRatio: '16 / 9', background: 'var(--t2-bg-2)',
  };
    ...
    <section style={wrap}>
      <div style={inner}>
        <div style={head}>
          ...
          <h2 style={h2}>Аликанте ALC → {r.ru}</h2>
          <p style={lede}>{r.time} мин в пути · {r.distance ? r.distance + ' · ' : ''}фиксированная цена за автомобиль</p>
        </div>
        <div style={frameWrap}>
          <iframe
            src={src}
            ...
            title={`Карта маршрута Аликанте ALC — ${r.ru}`}
          />
        </div>
      </div>
    </section>
...
function TripBreakdown({ r }) {
  const STEPS = [
    ...
    { icon: '🛣',  title: `Дорога ${r.time} мин`, text: 'Платные дороги уже оплачены. Чистый салон и климат-контроль.' },
  ...
  const TIERS = [
    {
      ...
      features: ['Кондиционер', 'Детское кресло'],
    ...
    },
    {
      ...
      features: ['Премиум-салон', 'Кожа', 'Climate control', 'Детское кресло'],
    ...
    },
    {
      ...
      features: ['Большой багажник', 'Климат-зона', 'Кресла для детей'],
      bg: '#fff', highlight: false,
    },
  ];
...
function DestinationTeaser({ r }) {
  ...
  const photoCard = { position: 'relative', borderRadius: 24, overflow: 'hidden', aspectRatio: '4 / 5', boxShadow: '0 30px 60px rgba(11,30,51,.18)', background: r.gradient || 'linear-gradient(135deg, var(--t2-deep), #0b1e33)' };
  const photoImg = { width: '100%', height: '100%', objectFit: 'cover' };
  const photoTag = { position: 'absolute', bottom: 18, left: 18, right: 18, padding: '14px 18px', borderRadius: 16, background: 'rgba(255,255,255,.92)', display: 'flex', alignItems: 'center', gap: 12 };
  ...
  // Fallback layer: huge city emoji + name on the brand gradient — looks
  // intentional instead of "empty box" when the photo is missing or 404s.
  const fallbackCenter = {
    position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column',
    alignItems: 'center', justifyContent: 'center', color: '#fff', textAlign: 'center',
    padding: '0 24px', textShadow: '0 2px 16px rgba(0,0,0,.35)',
  };
  const fallbackEmoji = { fontSize: 92, lineHeight: 1, marginBottom: 18, filter: 'drop-shadow(0 6px 18px rgba(0,0,0,.3))' };
  const fallbackName = { fontFamily: "'Onest',sans-serif", fontWeight: 800, fontSize: 36, letterSpacing: '-.02em', lineHeight: 1.05 };
  const fallbackSub = { fontSize: 13, opacity: .85, marginTop: 10, letterSpacing: '.06em', textTransform: 'uppercase' };

  // Legacy inline highlights — kept as a defaults bank for cities not yet in
  // CITY_INFO. CITY_INFO (BrandData.jsx) takes precedence when present.
  const HIGHLIGHTS = {
    Бенидорм: [
  ...
  // Pick highlights: prefer rich CITY_INFO from BrandData.jsx; fall back to
  // the inline legacy table; final fallback is a generic 3-bullet list.
  const items = r.info?.highlights || HIGHLIGHTS[r.ru] || [
  ...
  const teaserIntro = r.info?.intro || 'Мы знаем побережье как свои пять пальцев. Подскажем, куда сходить вечером, где поесть и какие места не пропустить.';
  const showImage = imgOk && r.img;
        ...
        <div style={split} className="t2-dest-split">
          ...
          <div>
            <div style={photoCard}>
              {showImage ? (
                <img
                  src={wikiThumb(r.img, 640)}
                  srcSet={wikiSrcset(r.img)}
                  sizes="(min-width: 900px) 560px, 100vw"
                  alt={r.alt || `${r.ru} — фото города`}
                  loading="lazy" decoding="async"
                  style={photoImg}
                  onError={() => setImgOk(false)} />
              ) : (
                <div style={fallbackCenter}>
                  <div style={fallbackEmoji}>{r.emoji}</div>
                  <div style={fallbackName}>{r.ru}</div>
                  <div style={fallbackSub}>Costa Blanca · Spain</div>
                </div>
              )}
              <div style={photoTag}>
        ...
        <div style={grid}>
          {all.slice(0, 5).map(r => (
            <div key={r.slug} style={card}
              ...
              <div style={thumb}>{r.img && <img src={wikiThumb(r.img, 160)} alt="" loading="lazy" decoding="async" width="56" height="56" style={thumbImg} />}</div>
              <div style={cText}>

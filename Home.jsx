...
function Home({ onNav, onOpenPost }) {
  ...
  return (
    <PageShell sidebar={<Sidebar onOpenPost={onOpenPost} />}>
      ...
      <p style={p}>
        Основными воздушными воротами в Аликанте является аэропорт Эльче. Именно сюда сразу попадают все прибывшие гости города. Такси в аэропорту Аликанте очень много. Здесь есть и белые авто с эмблемами разных компаний такси и чёрные автомобили — в основном индивидуальные перевозчики. Стоимость такси в Аликанте из аэропорта равна примерно 25-30 евро, в зависимости в какую точку города Вам надо доехать. Относительно дешево, как для европейского курорта. Заказать такси в Аликанте можно ещё до прибытия! Достаточно просто оставить <a onClick={() => onNav('routes')} style={{ color: 'var(--t2-red)', cursor: 'pointer' }}>онлайн заявку</a> на нашем сайте и наш опытный водитель будет ожидать Вас прямо при выходе из зоны получения багажа.
      </p>
      <p style={p}>
